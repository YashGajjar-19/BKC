...
const CommChannel = () =>
{
    ...
    // 1. Load Chats List OR All Members if Empty
    useEffect( () =>
    {
        if ( !user ) return;
        ...
        const unsubscribe = onSnapshot( q, async ( snapshot ) =>
        {
            ...
            {
                // If no active chats, show all members as a directory
                // We'll construct "fake" chat objects for display
                try
                {
                    ...
                    // Merge with static data for complete list
                    const combined = [ ...staticMembers ];
                    allUsers.forEach( u =>
                    {
                        const exists = combined.find( m => m.email === u.email || m.name === u.name );
                        if ( !exists ) combined.push( u );
                    } );

                    // Convert to chat-like structure for the sidebar
                    const directoryChats = combined
                        .filter( m => m.uid !== user.uid && m.id !== user.uid ) // Exclude self
                        .map( member => ( {
                            id: getChatId( user.uid, member.uid || member.id ),
                            type: 'direct',
                            participantData: {
                            ...
                            participants: [ user.uid, member.uid || member.id ],
                            lastMessage: { text: "Tap to start chatting", createdAt: null },
                            isDirectory: true // Flag to indicate this is just a contact
                        } ) );

                    setChats( directoryChats );
                } catch ( e )
                {
    ...
    const handleFileUpload = async (file) => {
        ...
        try {
            // 1. Upload to Storage
            const storageRef = ref(storage, `chat-media/${activeChat}/${Date.now()}_${file.name}`);
            const snapshot = await uploadBytes(storageRef, file);
            const downloadURL = await getDownloadURL(snapshot.ref);

            // 2. Prepare Message Data
            const msgData = {
                text: "", // No text for file-only messages for now
                senderId: user.uid,
                createdAt: serverTimestamp(),
